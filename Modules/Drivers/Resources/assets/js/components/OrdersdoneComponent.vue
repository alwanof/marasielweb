            ...
            <div class="card-body p-0 table-responsive">
                <table class="table table-striped table-hover table-head-fixed">
                    ...
                    <tr>
                        ...
                        <th>{{ local[lang+".orders"]["drivername"] }}</th>
    ...
    export default {
        ...
        methods: {
            getResults() {
                ...
                query.where('status','==','3').limit(pageSize).get()
                    .then(snap=>{
                        let total =0;
                        ...
                        snap.forEach(async doc=>{
                            ...
                            item.date=doc.data().order_date;
                                if(total<pageSize){
                                    ...
                                    .doc(doc.data().driver_uid).get()
                                    .then(user=>{
                                        item.driver=user.data().firstName+' '+user.data().lastName;
                                    });
                                    await CONFIG.DB.collection('users')
